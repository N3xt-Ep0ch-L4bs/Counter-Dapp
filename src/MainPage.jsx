...
function MainPage({ history, addHistory }) {
  ...
  const updateCount = (id, delta) => {
    setWallets(prev =>
      prev.map(wallet =>
        wallet.id === id
          ? (() => {
              const oldVal = wallet.count;
              const newVal = wallet.count + delta;
              addHistory(
    ...
    setWallets(prev =>
      prev.map(wallet =>
        wallet.id === id
          ? (() => {
              const oldVal = wallet.count;
              addHistory("Reset", oldVal, 0, connected && account?.address ? account.address : wallet.address);
  ...
  const deleteWallet = (id) => {
    ...
    if (target) {
      addHistory("Deleted", target.count, null, connected && account?.address ? account.address : target.address);
    }
    setWallets(prev => prev.filter(wallet => wallet.id !== id));
    if (openWalletId === id) setOpenWalletId(null);
    showToast('Wallet deleted successfully', 'error');
  };
        ...
        <div className="main-content">
          <div className="counter-box">
            ...
            <div className="btns">
              ...
              <button onClick={() => {
                addHistory("Increment", count, count + 1, connected && account?.address ? account.address : "0xGeneral");
                setCount(count + 1);
              }}>+</button>
              <button onClick={() => {
                addHistory("Decrement", count, count - 1, connected && account?.address ? account.address : "0xGeneral");
                setCount(count - 1);
              }}>-</button>
            </div>
          </div>
          ...
          <div className="wallets-section">
            <div className="wallets-header">
              ...
              <span className='history-link' onClick={() => navigate('/history')}>
